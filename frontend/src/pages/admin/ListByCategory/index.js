...
export default function ListByCategory() {
	...
	useEffect(() => {
		async function getCategory() {
			const response = await api.get(`/category/name/${search.category}`);
	...
	const saveEditCategory = async () => {
		if (verifyAllInputsValid()) {
			...
			const response = await api.put(`category/${category.id}`, new_data);
	...
	const deleteCategory = async () => {
		await api.delete(`/category/${categoryToDelete[0]}`);
			...
			</Container>
			{
				listCategory.rows !== undefined &&
					...
					<>
						...
						<Container className="margin_top_20 width_90 padding_all_10" fluid={true}>
							<ListGroupItem>
								<Row>
									<Col className="border_only_right padding_all_10 center border_color_gray" sm="2">
										<strong>Tombamento</strong>
									</Col>
									<Col className="border_only_right padding_all_10 center border_color_gray" sm="3">
									...
									return (
										<ListGroupItem className="margin_top_bottom_10" key={element.id}>
											<Row className="no_padding">
												...
												<Col className="border_only_right padding_all_10 center_vertical border_color_gray" sm="3">{element.description}</Col>
												<Col className="border_only_right padding_all_10 center_vertical border_color_gray" sm="2">{element.category.name}</Col>
												<Col className="border_only_right padding_all_10 center_vertical border_color_gray" sm="2">{element.belongs_department.acronym}</Col>
												<Col className="border_only_right padding_all_10 center_vertical border_color_gray" sm="1">{element.belongs_public_agency.acronym}</Col>
												<Col className="border_only_right padding_all_10 center border_color_gray" sm="1">
