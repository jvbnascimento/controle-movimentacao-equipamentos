...
export default function Department() {
	...
	useEffect(() => {
		async function getDepartment() {
			const response = await api.get(`/departments/name/${departmentName}`);
			const data = await response.data;
	...
	const toggleModalEditDepartment = () => {
		...
		setDepartmentBoss(departmentBoss);
	...
	const handleDepartmentName = (e) => {
		...
		department.name = verifyDepartmentName;
	...
	const saveEditDepartment = async () => {
		if (verifyAllInputsValid()) {
			...
			if (response.status === 200) {
				...
				setDepartmentName(departmentName.replace("/", "-"));
				history.push(`/department/${departmentName}`);
				toggleModalEditDepartment();
			...
			<Container fluid={true} className="width_70 margin_top_bottom_20">
				...
				<ListGroupItem>
					<Row className="text_left">
						...
						<Col sm="auto">
							<h6>{
								departmentBoss !== undefined ?
									departmentBoss.toUpperCase() :
									''
							}</h6>
						</Col>
					</Row>
